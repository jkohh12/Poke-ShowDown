...
public class TextureComparer : IComparer<Texture2D>
{
    public int Compare(Texture2D x, Texture2D y)
    {
        ...
        int xNumStart = FindNumberStart(xName);
        int yNumStart = FindNumberStart(yName);

        if (xNumStart != yNumStart)
        {
            return xNumStart.CompareTo(yNumStart);
        }

        return xName.CompareTo(yName);
    }

    private int FindNumberStart(string input)
    {
...
public class EditorHelper : MonoBehaviour
{
    ...
    static void SliceBattleSprites()
    {
        ...
        //Texture2D[] textures = new Texture2D[totalSprites];
        Texture2D[] textures = Resources.LoadAll<Texture2D>("FRONT").OrderBy(t => t, new TextureComparer()).ToArray();
        ...
        for (int z = 0; z < textures.Length; z++)//
        {
            ...
            TextureImporter ti_FR = AssetImporter.GetAtPath("Assets/Resources/FRONT/" + path) as TextureImporter;
            ...
            TextureImporter ti_BR = AssetImporter.GetAtPath("Assets/Resources/BACK/" + path) as TextureImporter;
            //Shiny
...
*/
       ...
       /*     if (ti_FS.textureType != TextureImporterType.Sprite) ti_FS.textureType = TextureImporterType.Sprite;
            ...
            if (ti_FR.spriteImportMode == SpriteImportMode.Multiple)
            {
                ...
                AssetDatabase.ImportAsset("Assets/Resources/FRONT/" + path, ImportAssetOptions.ForceUpdate);
            }
            if (ti_BR.spriteImportMode == SpriteImportMode.Multiple)
            {
                // Bug? Need to convert to single then back to multiple in order to make changes when it's already sliced
                ti_BR.spriteImportMode = SpriteImportMode.Single;
                AssetDatabase.ImportAsset("Assets/Resources/BACK/" + path, ImportAssetOptions.ForceUpdate);
            }
